...
const Twitter: React.FC<{
  ...
  const handleAddConnect = async () => {
    try {
      ...
      await updateUser(userId, { x: twitterDetails });

      SetTwitterUsername(twitterDetails.username);
      setIsUserConnected(true);
      NotificationMessage("success", "X profile Linked");
    } catch (error) {
      console.error("Error updating Twitter details:", error);
      setIsUserConnected(false);
      onFailure();
    }
  };
  ...
  return (
    <div className='social_bx'>
      ...
      <div className='connect_btn' onClick={handleAddConnect}>
